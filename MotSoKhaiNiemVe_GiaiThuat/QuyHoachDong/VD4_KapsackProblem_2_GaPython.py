...
**Note** mỗi đồ vật có thể lấy được nhiều lần
...
for i_n in range(1,n+1):
    for L_b in range(1,b+1):
        ...
        if L_b >= w[i_n]:
            result[i_n][L_b] = max(result[i_n - 1][L_b] , result[i_n][L_b - w[i_n]] + c[i_n])
print(result[n][b])
for x in range(n+1):
    print(result[x])

#6. ghi ra file:
fo.write(f'result: {result[n][b]} \t')


#7. truy vết vật mang theo (show tổng các vật trong balo)
i_n = n
L_b = b
lst_i_n = []
while i_n:
    if result[i_n][L_b] == max(result[i_n - 1][L_b] , result[i_n][L_b - w[i_n]] + c[i_n]):
        lst_i_n.append(i_n)
        L_b = L_b - w[i_n]
    i_n -= 1
for i in lst_i_n:
    print('weight:',c[i],end=" ")
    #7. ghi ra file:
    fo.write(f'weight: {w[i]}\t')